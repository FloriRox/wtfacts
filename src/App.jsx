...
function dbListen(code, fn) { return onValue(dbRef(code), snap => fn(snap.val())); }
...
const FB_ADULT={question:"Wie viel Liter Blut pumpt das menschliche Herz pro Tag?",answer:7200,unit:"Liter",hint:"Das Herz schlägt ca. 100.000 Mal am Tag und pumpt dabei etwa 70ml pro Schlag.",topic:"Körper",emoji:"❤️"};
...
function calcRound(room) {
  ...
  const ranked=order.filter(id=>guesses[id]!=null).map(id=>({id,diff:Math.abs(guesses[id]-q.answer)})).sort((a,b)=>a.diff-b.diff);
...
function QRCode({url,t}) {
  const qrUrl=`https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(url)}&bgcolor=${t.id==="adult"?"1e1a17":"ffffff"}&color=${t.id==="adult"?"e8360a":"ff5c5c"}`;
  return (
    <div style={{textAlign:"center",marginTop:16}}>
      <p style={{fontSize:11,fontWeight:700,color:t.muted,letterSpacing:.8,marginBottom:10}}>EINLADUNGS-QR</p>
      <img src={qrUrl} alt="QR Code" style={{width:140,height:140,borderRadius:t.radius,border:`2px solid ${t.border}`}}/>
      <p style={{fontSize:12,color:t.muted,marginTop:8}}>Scannen zum Beitreten</p>
    </div>
  );
}
...
function Btn({children,onClick,variant="primary",disabled,t,full,style:sx={}}){
  ...
  const variants={primary:{background:t.accent,color:"#fff"},secondary:{background:"transparent",color:t.text,border:`2px solid ${t.border}`},ghost:{background:"transparent",color:t.muted}};
...
function QuestionScreen({room,myId,t,onGuess}){
  ...
  return <div style={page}>
    ...
    <Card t={t} glow style={{marginBottom:14,animation:"fu .3s .05s ease both"}}>
      ...
      <p style={{fontSize:t.id==="kids"?20:18,lineHeight:1.55,fontWeight:t.id==="kids"?700:500,marginTop:12}}>{q.question}</p>
...
function ResultsScreen({room,myId,t,onNext,onEnd}){
  ...
  const isHost=room.hostId===myId;
  const medals=["🥇","🥈","🥉"];
  const ranked=pl.filter(p=>guesses[p.id]!=null).map(p=>({...p,guess:guesses[p.id],diff:Math.abs(guesses[p.id]-q.answer)})).sort((a,b)=>a.diff-b.diff);
  ...
  return <div style={page}>
    <div style={{textAlign:"center",marginBottom:22,animation:"fu .3s ease both"}}>
      ...
      <div style={{fontFamily:t.fontTitle,fontSize:"clamp(50px,12vw,82px)",color:t.accent,lineHeight:1,marginTop:8,animation:"pop .5s ease both",textShadow:t.id==="adult"?"0 0 32px rgba(232,54,10,.35)":undefined}}>{fmtNum(q.answer)} {q.unit}</div>
    ...
    {Object.keys(bets).length>0&&<Card t={t} style={{marginBottom:12,animation:"fu .3s .12s ease both"}}>
...
export default function App(){
  ...
  async function handleStart(){
    setLoading(true);setLoadTxt(mode==="kids"?"🤔 Frage wird generiert...":"KI generiert eine Frage...");
  ...
  async function handleNext(){
    setLoading(true);setLoadTxt(mode==="kids"?"🤔 Neue Frage...":"Nächste Frage kommt...");
    const q=await generateQuestion(mode,topicsRef.current);
    topicsRef.current.push(q.topic);
    setLoading(false);
  ...
  return <>
    ...
    {screen==="lobby"&&room&&<LobbyScreen room={room} code={code} myId={myId} t={t} onStart={handleStart}/>}
